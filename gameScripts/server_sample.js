...
function handleUserInput_main() {
    ...
    switch (choice) {
        case '1': // 새로운 게임 시작
            ...
            game_main.startGame(game_main.battle_status.Battle_statusTOJson(game_main.battle_status),
                init_game.game_setting.game_difficluty);
            break;
        ...
        case flag_game_start ? '3' : '11': // 게임 저장
            if (flag_game_start) {
                try {
                    ...
                    fs.writeFileSync(`${save_file}/${Date.now()}_${takeinput('입력 : ', ' 저장할 이름을 설정해주세요.')}.txt`, game_main.battle_status.Battle_statusTOJson(game_main.battle_status));
...
// 게임 시작 함수
function start(flag_mid_game, battle_status) {
    flag_game_start = flag_mid_game ?? flag_game_start;
    init_game.game_setting = init_game.read_init();
    game_main.battle_status = battle_status ?? new game_main.Battle_status();
    displayLobby();
    handleUserInput_main();
}
    ...
    try {
        ...
        if (dir_file_list.length > 0) {
            ...
            dir_file_list.forEach((element) => {
                console.log(chalk.blue(`${i++}. `) + chalk.white(element));
            });
        }
        else {
...
X 를 입력해 메뉴로 돌아가기`);
        ...
        switch (choice) {
            case 'X' || 'x': // 메뉴로 돌아가기
                ...
                break;
            default:
                if (choice.length === 0 || +choice < 0 || +choice > dir_file_list.length) {
                    console.log(chalk.red('올바른 선택을 하세요.'));
                    ...
                    try {
                        ...
                        init_game.sleep(1000);
                        const tmp_saved_data = fs.readFileSync(`${save_file}/${dir_file_list[choice]}`);
                        flag_game_start = true;
                        game_main.startGame(tmp_saved_data);
                    }
                    catch (err) {
                        console.log(chalk.red('게임이 정상적으로 저장되지 않았습니다.'));
                        console.error(err);
...
///업적 표시하기
function displayAchievementPage() {
    ...
    try {
        ...
        achievement.achievement_list.forEach((element, index) => {
            if (element.achieved == true)
                console.log(chalk.white(index + '.' + element.name));
            else
                console.log(chalk.gray(index + '.' + element.name));
        });
        // 옵션들
        let choice = takeinput('입력 : ', `엔터를 입력해 메뉴로 돌아가기`);
        start();
    }
    catch (err) {
        console.log(chalk.red('에러가 발생했습니다. 개발자에게 신고하세요.'));
